...
public class TurretSubsystem extends SubsystemWithMapleSimSimulation {
	...
	private static final double kLookaheadSeconds = 0.020;
	...
	public Command autoAimCommand(
		...
		return run(() -> {
			...
			Pose2d predictedPose = PosePrediction.Linear(robotPose, speeds, kLookaheadSeconds);

			// TurretUtil.ShotSolution solution = TurretUtil.computeLeadingShot(predictedPose, speeds, target.get());
			TurretUtil.ShotSolution solution = TurretUtil.computeShotSolution(predictedPose, target.get());
